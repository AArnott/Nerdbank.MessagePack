...
partial class MessagePackPrimitives
{
	...
	/// </summary>
	/// <param name="source">The buffer to read from.</param>
	/// <param name="count">Receives the number of key-value pairs in the array, if successful.</param>
	/// <param name="readTo">Receives the position of the last byte consumed from <paramref name="source"/>.</param>
	/// <returns>The result classification of the read operation.</returns>
	/// <remarks>
	/// Reads a array header from
	/// <see cref="MessagePackCode.Array16"/>,
	/// <see cref="MessagePackCode.Array32"/>, or
	/// some built-in code between <see cref="MessagePackCode.MinFixArray"/> and <see cref="MessagePackCode.MaxFixArray"/>.
	/// </remarks>
	public static DecodeResult TryReadArrayHeader(ReadOnlySequence<byte> source, out uint count, out SequencePosition readTo)
	{
		...
		return SlowPath(source, result, tokenSize, ref count, out readTo);
		static DecodeResult SlowPath(ReadOnlySequence<byte> source, DecodeResult decodeResult, int tokenSize, ref uint count, out SequencePosition readTo)
		{
			switch (decodeResult)
			{
				case DecodeResult.Success:
					readTo = source.GetPosition(tokenSize);
					return decodeResult;
				case DecodeResult.InsufficientBuffer or DecodeResult.EmptyBuffer when source.Length >= tokenSize:
					// There is enough buffer. Just not in contiguous memory.
					Span<byte> span = stackalloc byte[tokenSize];
					source.Slice(0, tokenSize).CopyTo(span);
					decodeResult = TryReadArrayHeader(span, out count, out tokenSize);
					readTo = source.GetPosition(tokenSize);
					return decodeResult;
				default:
					readTo = source.Start;
		...
		static DecodeResult SlowPath(ReadOnlySequence<byte> source, DecodeResult decodeResult, int tokenSize, ref uint count, out SequencePosition readTo)
		{
			switch (decodeResult)
			{
				...
				case DecodeResult.InsufficientBuffer or DecodeResult.EmptyBuffer when source.Length >= tokenSize:
					...
					Span<byte> span = stackalloc byte[tokenSize];
					source.Slice(0, tokenSize).CopyTo(span);
					decodeResult = TryReadMapHeader(span, out count, out tokenSize);
					readTo = source.GetPosition(tokenSize);
					return decodeResult;
				default:
					readTo = source.Start;
	...
	/// </remarks>
	public static DecodeResult TryRead(ReadOnlySpan<byte> source, out bool value, out int tokenSize)
	{
		tokenSize = 1;
		if (source.Length == 0)
		{
	...
	{
		DecodeResult result = TryRead(source, out ushort ordinal, out tokenSize);
		if (result == DecodeResult.Success)
		{
			value = (char)ordinal;
		}
	...
	/// </remarks>
	public static DecodeResult TryRead(ReadOnlySpan<byte> source, out DateTime value, out int tokenSize)
	{
	...
	/// <summary>
	/// Contains jump tables that translate a msgpack byte directly to a method that reads an integer of the corresponding type.
	/// </summary>
	/// <remarks>
	/// These jump tables are *very* fast. They cost only an array lookup (virtually free) and a virtual method call (quite cheap).
	/// It is faster than a switch statement because it avoids the need to compare the byte to each possible value or range.
	/// It is also faster than using delegates, because the delegate call is more expensive than a virtual method call.
	/// </remarks>
	static partial class Decoders
	{
		...
		/// <summary>
		/// An interface implemented for methods that read a signed 64-bit integer from a buffer.
		/// </summary>
		internal interface IReadInt64
		{
			/// <summary>
			/// Reads a signed 64-bit integer from the specified buffer.
			/// </summary>
			/// <param name="source">The buffer to read from.</param>
			/// <param name="value">Receives the value if successful.</param>
			/// <param name="tokenSize">Receives the number of bytes read from the source, or the minimum length of <paramref name="source"/> required to read the data.</param>
		...
		/// <summary>
		/// An interface implemented for methods that read an unsigned 64-bit integer from a buffer.
		/// </summary>
		internal interface IReadUInt64
		{
			/// <summary>
			/// Reads an unsigned 64-bit integer from the specified buffer.
			/// </summary>
			/// <param name="source">The buffer to read from.</param>
			/// <param name="value">Receives the value if successful.</param>
			/// <param name="tokenSize">Receives the number of bytes read from the source, or the minimum length of <paramref name="source"/> required to read the data.</param>
