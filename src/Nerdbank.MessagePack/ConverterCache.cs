...
internal record class ConverterCache
{
	...
	internal void RegisterConverter<T>(MessagePackConverter<T> converter)
	{
		...
		this.userProvidedConverters[typeof(T)] = this.PreserveReferences != ReferencePreservationMode.Off
	...
	private void ReconfigureUserProvidedConverters()
	{
		foreach (KeyValuePair<Type, object> pair in this.userProvidedConverters)
		{
			...
			this.userProvidedConverters[pair.Key] = this.PreserveReferences != ReferencePreservationMode.Off
