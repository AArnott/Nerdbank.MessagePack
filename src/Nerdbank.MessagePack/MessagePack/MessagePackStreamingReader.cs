...
public ref partial struct MessagePackStreamingReader
{
	...
	public DecodeResult TryReadBinHeader(out uint length)
	{
		...
		DecodeResult readResult = MessagePackPrimitives.TryReadBinHeader(this.reader.UnreadSpan, out length, out int tokenSize);
		if (readResult == DecodeResult.Success)
		{
			this.Advance(tokenSize, 0);
			return DecodeResult.Success;
		...
		static DecodeResult SlowPath(ref MessagePackStreamingReader self, DecodeResult readResult, bool usingBinaryHeader, ref uint length, ref int tokenSize)
		{
			switch (readResult)
			{
				case DecodeResult.Success:
