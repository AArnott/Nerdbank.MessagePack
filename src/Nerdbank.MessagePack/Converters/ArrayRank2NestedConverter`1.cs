...
/// <summary>
/// Converts 2-dimensional arrays by nesting them into individual msgpack arrays for serialization.
/// </summary>
...
/// <c>[[0-0, 0-1, ...], [1-0, 1-1, ...]]</c>.
/// </remarks>
internal class ArrayRank2NestedConverter<TElement>(MessagePackConverter<TElement> elementConverter) : MessagePackConverter<TElement[,]>
{
...
#pragma warning restore NBMsgPack031 // Converters should read or write exactly one msgpack structure
	...
	public override void Write(ref MessagePackWriter writer, in TElement[,]? value, SerializationContext context)
	{
		...
		for (int i = 0; i < length0; i++)
		{
			...
			for (int j = 0; j < length1; j++)
			{
				elementConverter.Write(ref writer, value[i, j], context);
			}
		}
	}
