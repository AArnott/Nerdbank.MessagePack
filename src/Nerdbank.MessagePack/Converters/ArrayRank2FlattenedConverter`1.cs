...
/// <summary>
/// Converts 2-dimensional arrays by flattening them into a single array for serialization.
/// </summary>
...
/// </remarks>
internal class ArrayRank2FlattenedConverter<TElement>(MessagePackConverter<TElement> elementConverter) : MessagePackConverter<TElement[,]>
{
...
#pragma warning restore NBMsgPack031 // Converters should read or write exactly one msgpack structure
	{
		...
		for (int i = 0; i < dimensionLengths[0]; i++)
		{
			...
			{
				result[i, j] = elementConverter.Read(ref reader, context)!;
			}
		}
	...
	public override void Write(ref MessagePackWriter writer, in TElement[,]? value, SerializationContext context)
	{
		...
		writer.WriteArrayHeader(length0 * length1);
		for (int i = 0; i < length0; i++)
		{
			for (int j = 0; j < length1; j++)
			{
				elementConverter.Write(ref writer, in value[i, j], context);
			}
		}
	}
