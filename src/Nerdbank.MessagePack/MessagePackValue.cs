...
[GenerateShape]
public partial struct MessagePackValue : IEquatable<MessagePackValue>
{
	/// <summary>
	...
	private readonly float float32;

	private readonly double float64;
...
#if NET
	private readonly ReadOnlyMemory<MessagePackValue> array;
#endif
	...
	public MessagePackValue(sbyte value)
	{
		...
		this.integer = unchecked((ulong)value);
	}
	...
	public MessagePackValue(ReadOnlyMemory<byte> value)
	{
	...
	public MessagePackValue(ReadOnlyMemory<MessagePackValue> value)
	{
...
#pragma warning disable SA1623 // Property summary documentation should match accessors
	public bool ValueAsBoolean => this.kind == MessagePackValueKind.Boolean ? this.boolean : this.ThrowInvalidCastException<bool>();
#pragma warning restore SA1623 // Property summary documentation should match accessors
	...
	{
		MessagePackValueKind.Single => this.float32,
		MessagePackValueKind.Double => checked((float)this.float64),
	...
	{
		MessagePackValueKind.Single => this.float32,
		MessagePackValueKind.Double => this.float64,
		MessagePackValueKind.SignedInteger => this.SignedInteger,
		MessagePackValueKind.UnsignedInteger => this.UnsignedInteger,
	...
	public ReadOnlyMemory<byte> ValueAsBinary => this.kind == MessagePackValueKind.Binary ? this.binary : this.ThrowInvalidCastException<ReadOnlyMemory<byte>>();

	/// <summary>
	/// Gets the value as an array.
	/// </summary>
	/// <exception cref="InvalidCastException">Thrown when the value is not an array.</exception>
	public ReadOnlyMemory<MessagePackValue> ValueAsArray => this.kind == MessagePackValueKind.Array ? this.array : this.ThrowInvalidCastException<ReadOnlyMemory<MessagePackValue>>();
	...
	public Extension ValueAsExtension => this.kind == MessagePackValueKind.Extension ? this.extension : this.ThrowInvalidCastException<Extension>();
	...
	public DateTime ValueAsDateTime => this.kind == MessagePackValueKind.DateTime ? this.dateTime : this.ThrowInvalidCastException<DateTime>();
	...
	{
		MessagePackValueKind.SignedInteger => unchecked((long)this.integer),
		MessagePackValueKind.UnsignedInteger => checked((long)this.integer),
	...
	{
		MessagePackValueKind.UnsignedInteger => this.integer,
		MessagePackValueKind.SignedInteger => checked((ulong)unchecked((long)this.integer)),
...
#if !NET
	// .NET Framework does not support struct fields with generic type arguments to themselves.
#pragma warning disable SA1300 // Element should begin with upper-case letter
	private MessagePackValue[] array => (MessagePackValue[])this.refValue!;
#pragma warning restore SA1300 // Element should begin with upper-case letter
	...
	public static implicit operator MessagePackValue(byte[]? value) => value is null ? Nil : new((ReadOnlyMemory<byte>)value);

	/// <summary>
	/// Implicitly converts binary data to a <see cref="MessagePackValue"/>.
	/// </summary>
	/// <param name="value">The binary data value to convert.</param>
	public static implicit operator MessagePackValue(ReadOnlyMemory<byte> value) => new(value);
	...
	/// </summary>
	/// <param name="value">The array to convert.</param>
	public static implicit operator MessagePackValue(ReadOnlyMemory<MessagePackValue> value) => new(value);
#endif
	...
	public override string ToString()
		=> this.kind switch
		{
			...
			MessagePackValueKind.Boolean => this.boolean ? "true" : "false",
			MessagePackValueKind.Single => this.float32.ToString("R", CultureInfo.InvariantCulture),
			MessagePackValueKind.Double => this.float64.ToString("R", CultureInfo.InvariantCulture),
			MessagePackValueKind.String => (string)this.refValue!,
			MessagePackValueKind.Binary => $"byte[{this.binary.Length}]",
			MessagePackValueKind.Array => $"array[{this.array.Length}]",
			MessagePackValueKind.Map => $"map[{((IReadOnlyDictionary<MessagePackValue, MessagePackValue>)this.refValue!).Count}]",
			MessagePackValueKind.Extension => $"extension type {this.extension.TypeCode}",
			MessagePackValueKind.DateTime => this.dateTime.ToString(CultureInfo.InvariantCulture),
	...
	public bool Equals(MessagePackValue other)
	{
		...
		return this.kind switch
		{
			...
			MessagePackValueKind.UnsignedInteger or MessagePackValueKind.SignedInteger => this.integer == other.integer,
			MessagePackValueKind.Boolean => this.boolean == other.boolean,
			MessagePackValueKind.Single => this.float32 == other.float32,
			MessagePackValueKind.Double => this.float64 == other.float64,
			MessagePackValueKind.String => (string?)this.refValue == (string?)other.refValue,
			MessagePackValueKind.Binary => MemoryEquals(this.binary, other.binary),
#if NET
			MessagePackValueKind.Array => MemoryEquals(this.array, other.array),
#else
			MessagePackValueKind.Array => this.array == other.array || this.array.SequenceEqual(other.array),
#endif
			...
			MessagePackValueKind.Extension => this.extension.Equals(other.extension),
			MessagePackValueKind.DateTime => this.dateTime != other.dateTime,
			_ => throw new NotImplementedException(),
		};

		static bool MemoryEquals<T>(ReadOnlyMemory<T> left, ReadOnlyMemory<T> right)
			where T : IEquatable<T>
		{
			...
			if (MemoryMarshal.TryGetArray(left, out ArraySegment<T> leftSegment) &&
				...
				leftSegment.Equals(rightSegment))
			{
				// The memory blocks are pointing to the same location in memory.
				return true;
			}
