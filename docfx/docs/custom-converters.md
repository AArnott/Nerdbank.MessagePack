...
> [!CAUTION]
> It is imperative that each `Write` and `Read` method write and read _exactly one_ msgpack structure.
...
When reading arrays, you must read _all_ the values in the array, even if you don't expect more than some given number of elements.
...
It also implicitly skips values in any unknown array index, such that reading _all_ array elements is guaranteed.
...
Modify state on an existing @ShapeShift.MessagePackSerializer by capturing the context as a local variable, mutating state there, then creating a new serializer with the modified state, as follows:
