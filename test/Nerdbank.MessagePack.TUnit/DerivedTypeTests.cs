...
public partial class DerivedTypeTests : MessagePackSerializerTestBase
{
	...
	public async Task UnrecognizedAlias_PreservesDiscriminatorAndData(bool async, bool useDiscriminatorObjects, bool useStringAlias)
	{
		...
		VersionTolerantBase? value = async
			? await this.Serializer.DeserializeAsync<VersionTolerantBase>(PipeReader.Create(sequence), this.TimeoutToken)
			: this.Serializer.Deserialize<VersionTolerantBase>(sequence, this.TimeoutToken);
		...
		if (async)
		{
			...
			await this.Serializer.SerializeAsync<VersionTolerantBase>(stream, value, this.TimeoutToken);
			roundtripped = stream.ToArray();
		}
		else
		{
			roundtripped = this.Serializer.Serialize<VersionTolerantBase>(value, this.TimeoutToken);
		}
	...
	public void UnrecognizedAlias_PreservedDataIsRecognizedByNewerMapping()
	{
		...
		VersionTolerantBase? oldVersionValue = this.Serializer.Deserialize<VersionTolerantBase>(sequence, this.TimeoutToken);
		Assert.NotNull(oldVersionValue);

		byte[] preservedData = this.Serializer.Serialize<VersionTolerantBase>(oldVersionValue, this.TimeoutToken);
		...
		VersionTolerantNewDerived? newerVersionValue = Assert.IsType<VersionTolerantNewDerived>(this.Serializer.Deserialize<VersionTolerantBase>(preservedData, this.TimeoutToken));
	...
	public void UnrecognizedAlias_PreservesArrayPayload()
	{
		...
		VersionTolerantArrayBase? value = this.Serializer.Deserialize<VersionTolerantArrayBase>(sequence, this.TimeoutToken);
		Assert.NotNull(value);
		Assert.Equal(1, value.BaseProperty);

		byte[] roundtripped = this.Serializer.Serialize<VersionTolerantArrayBase>(value, this.TimeoutToken);
		MessagePackReader reader = new(roundtripped);
		Assert.Equal(2, reader.ReadArrayHeader());
		Assert.Equal(100, reader.ReadInt32());
		Assert.Equal(2, reader.ReadArrayHeader());
	...
	public void UseDiscriminatorObjects_MatchesExpectedFormat()
	{
		...
		byte[] msgpack = this.Serializer.Serialize<DynamicallyRegisteredBase>(new DynamicallyRegisteredDerivedA(), this.TimeoutToken);
