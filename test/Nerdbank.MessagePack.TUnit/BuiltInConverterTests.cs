...
public partial class BuiltInConverterTests : MessagePackSerializerTestBase
{
	...
	public void Int128_FromBin()
	{
		...
		Assert.Equal(value, this.Serializer.Deserialize<HasInt128>(seq, this.TimeoutToken)!.Value);
	}
	...
	public void UInt128_FromBin()
	{
		...
		Assert.Equal(value, this.Serializer.Deserialize<HasUInt128>(seq, this.TimeoutToken)!.Value);
	}
	...
	public void Decimal_FromBin()
	{
		...
		Assert.Equal(value[0], this.Serializer.Deserialize<HasDecimal>(seq, this.TimeoutToken)!.Value);
	}
	...
	public void Decimal_FromString()
	{
		...
		Assert.Equal(value, this.Serializer.Deserialize<HasDecimal>(seq, this.TimeoutToken)!.Value);
	}
	...
	public void BigInteger_FromBin()
	{
		...
		Assert.Equal(value, this.Serializer.Deserialize<HasBigInteger>(seq, this.TimeoutToken)!.Value);
	}
	...
	{
		byte[] msgpack = this.Serializer.Serialize<CultureInfo, Witness>(CultureInfo.GetCultureInfo("es-ES"), this.TimeoutToken);
	...
	public void Guid_FromBin()
	{
		...
		Assert.Equal(valueSpan[0], this.Serializer.Deserialize<HasGuid>(seq, this.TimeoutToken)!.Value);
	}
	...
	public void ByteArrayWithOldSpec()
	{
		...
		this.Serializer.Serialize<byte[], Witness>(ref writer, original, this.TimeoutToken);
		writer.Flush();
		...
		byte[]? deserialized = this.Serializer.Deserialize<byte[], Witness>(seq, this.TimeoutToken);
	...
	private (Guid Before, Guid After) RoundtripModifiedGuid(Func<string, string> modifier, MessagePackSerializer? serializer = null, MessagePackSerializer? deserializer = null)
	{
		...
		byte[] msgpack = serializer.Serialize<Guid, Witness>(original, this.TimeoutToken);
		msgpack = this.Serializer.Serialize<string, Witness>(
			modifier(this.Serializer.Deserialize<string, Witness>(msgpack, this.TimeoutToken)!),
			this.TimeoutToken);
		this.LogMsgPack(msgpack);
		Guid deserialized = deserializer.Deserialize<Guid, Witness>(msgpack, this.TimeoutToken);
