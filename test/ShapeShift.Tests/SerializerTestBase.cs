...
public abstract class SerializerTestBase(SerializerBase serializer)
{
	protected ReadOnlySequence<byte> lastRoundtrippedFormattedBytes;
	...
	protected SerializerBase Serializer { get; set; } = serializer;
...
#endif
	{
		...
		return this.lastRoundtrippedFormattedBytes;
	}
	...
	protected T? Roundtrip<T>(T? value, ITypeShape<T> shape)
	{
		...
		this.lastRoundtrippedFormattedBytes = sequence;
	...
	protected async ValueTask<T?> RoundtripAsync<T>(T? value, ITypeShape<T> shape)
	{
		...
		{
			this.lastRoundtrippedFormattedBytes = loggingSequence;
			this.LogFormattedBytes(loggingSequence);
		}
	}
}
