...
public partial class DeserializePathTests : MessagePackSerializerTestBase
{
	...
	[Theory, PairwiseData]
	public void PathNotFound_Throws(bool nullRoot, bool nullMidPath)
	{
		object container = nullRoot
			? (object)null!
			: new OuterContainer(nullMidPath ? null : new(5));
	...
	[Theory, PairwiseData]
	public void PathNotFound_ReturnsDefault(bool nullRoot, bool nullMidPath)
	{
		object container = nullRoot
			? (object)null!
			: new OuterContainer(nullMidPath ? null : new(5));
	...
	public void ComplexPath()
	{
		...
		MessagePackSerializer.DeserializePathOptions<ComplexContainer, int> options = new(c => c.Outer!.Values["key2"]![1]);
		int result = this.Serializer.DeserializePath<ComplexContainer, int, Witness>(msgpack, options, TestContext.Current.CancellationToken);
	...
	{
		ContainerWithStruct container = new(new StructValue(100, "test"));
		byte[] msgpack = this.Serializer.Serialize<ContainerWithStruct, Witness>(container, TestContext.Current.CancellationToken);
		this.LogMsgPack(msgpack);

		MessagePackSerializer.DeserializePathOptions<ContainerWithStruct, StructValue> options = new(c => c.Data);
		StructValue result = this.Serializer.DeserializePath<ContainerWithStruct, StructValue, Witness>(msgpack, options, TestContext.Current.CancellationToken);
	...
	[GenerateShape]
	public partial record struct StructValue(int Id, string Name);
	...
	[GenerateShape]
	public partial record DictionaryContainer(Dictionary<string, int[]?> Values);
	...
	[GenerateShape]
	public partial record ContainerWithStruct(StructValue Data);
