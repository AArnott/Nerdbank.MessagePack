...
public partial class AllocationAmplificationTests
{
	...
	public class Attacks : AllocationAmplificationTests
	{
		...
		public void RecursiveArrayType_DoesNotAmplifyAllocation()
		{
			...
			AssertRejectedWithinAllocationBudget<MessagePackSerializationException>(
				() => serializer.Deserialize<ArrayNode>(payload, TestContext.Current.CancellationToken),
		...
		public void ObjectConverter_DoesNotAmplifyAllocation()
		{
			...
			AssertRejectedWithinAllocationBudget<MessagePackSerializationException>(
				() => serializer.Deserialize<object, Witness>(payload, TestContext.Current.CancellationToken),
				allocationBudget: 64 * 1024,
				payload.Length);
		}
		...
		public async Task RecursiveArrayType_Async_DoesNotAmplifyAllocation()
		{
			...
			async Task DeserializeAsync()
			{
				...
				try
				{
					await serializer.DeserializeAsync<ArrayNode>(pipeReader, TestContext.Current.CancellationToken);
				}
				finally
				{
	...
	public class PositiveCompatibility : AllocationAmplificationTests
	{
		...
		public void DeepValidObjectGraph_RoundTrips()
		{
			...
			MessagePackSerializer serializer = new();
			byte[] msgpack = serializer.Serialize(root, TestContext.Current.CancellationToken);
			ArrayNode? result = serializer.Deserialize<ArrayNode>(msgpack, TestContext.Current.CancellationToken);
		...
		public void WideValidArray_RoundTrips()
		{
			...
			MessagePackSerializer serializer = new();
			byte[] msgpack = serializer.Serialize<int[], Witness>(values, TestContext.Current.CancellationToken);
			Assert.Equal(values, serializer.Deserialize<int[], Witness>(msgpack, TestContext.Current.CancellationToken));
		}
		...
		public void WideValidDictionary_RoundTrips()
		{
			...
			MessagePackSerializer serializer = new();
			byte[] msgpack = serializer.Serialize<Dictionary<int, int>, Witness>(values, TestContext.Current.CancellationToken);
			Assert.Equal(values, serializer.Deserialize<Dictionary<int, int>, Witness>(msgpack, TestContext.Current.CancellationToken));
		}
